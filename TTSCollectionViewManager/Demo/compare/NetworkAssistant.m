...
- (void)handleByURL:(NSString *)url
    ...
    NSString *cacheKey = [self getMD5StringFromNSString:[url stringByAppendingString:params?[params description]:@""]];
    ...
    NSLog(@"request header = %@ url = %@",manager.requestSerializer.HTTPRequestHeaders,url);
    NSLog(@"params = %@",params);
...
//    else
    ...
    @autoreleasepool {
        if (!rootObject) {
            ...
            NSLog(@"errorcode = %@",[rootObject objectForKey:@"errorCode"]);

...
- (void)createBaseDirectoryAtPath:(NSString *)path {
    ...
    if (error) {
        BBNLog(@"create cache directory failed, error = %@", error);
    } else {
        [BBNetworkPrivate addDoNotBackupAttribute:path];
    }
}
...
- (NSString *)cacheBasePath {
    ...
    NSArray *filters = [[BBNetworkConfig sharedInstance] cacheDirPathFilters];
    if (filters.count > 0) {
        for (id<BBCacheDirPathFilterProtocol> f in filters) {
...
- (NSString *)cacheFileName {
    ...
    NSString *baseUrl = [BBNetworkConfig sharedInstance].baseUrl;
    ...
    NSString *requestInfo = [NSString stringWithFormat:@"Method:%ld Host:%@ Url:%@ Argument:%@ AppVersion:%@ Sensitive:%@",
                             ...
                             argument, [BBNetworkPrivate appVersionString], [self cacheSensitiveData]];
    NSString *cacheFileName = [BBNetworkPrivate md5StringFromString:requestInfo];
...
- (int)cacheFileDuration:(NSString *)path {
    ...
    if (!attributes) {
        BBNLog(@"Error get attributes for file at %@: %@", path, attributesRetrievalError);
        return -1;
    }
...
- (NSDictionary *)requestHeaderFieldValueDictionary {
    return [[BBNetworkConfig sharedInstance] requestHeaderFieldValueDictionary];
}
@end
